...
async def _send_webhook_once(payment_id: uuid.UUID) -> None:
    # The row lock makes the webhook_sent_at check-and-set atomic: a duplicate
    # delivery blocks here until the first sender commits, then sees
    # webhook_sent_at and skips. The lock is held during the HTTP request, but
    # it only contends with duplicates of the same payment.
    ...
    if webhook_error is not None:
        raise RuntimeError(
            f"Webhook delivery failed for payment {payment_id}: {webhook_error}",
        ) from webhook_error
